...
```bash
npm install @agenite/provider-openai
```
...
```bash
npm install @agenite/provider-anthropic
```
...
```bash
npm install @agenite/provider-bedrock
```
...
```bash
npm install @agenite/provider-ollama
```
...
5. **Content mapping**: Properly map provider-specific responses to Agenite's standard format
