...
**Providers** in Agenite are implementations of the `LLMProvider` interface that connect your agents to specific large language model services. They translate the standardized Agenite messaging format into provider-specific API calls and handle the response processing back into the Agenite format.
...
```typescript
import { Anthropic } from '@agenite/provider-anthropic';
...
```typescript
import { Bedrock } from '@agenite/provider-bedrock';
...
```typescript
import { Ollama } from '@agenite/provider-ollama';
